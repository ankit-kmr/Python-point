...
def CopyFromWin(Win_path ,Linux_path ):
    ...
    for filesN in os.listdir(Win_path):
        ...
        if os.path.isfile(w_path):
            ...
            print('... Directory Detected ... {',filesN,'}')
            try:
                ftp_client.chdir(U_path)
            except IOError:
                print('+ + + ', U_path, ' + + +')
                ftp_client.mkdir(U_path)
                
            CopyFromWin(w_path ,U_path)


if __name__ == '__main__':
    
    global ftp_client ,ssh_client
    ...
    CopyFromWin (Win_path ,Linux_path)
